...
def volkswagen_mqb_meb_gen2_checksum(address: int, sig, d: bytearray) -> int:
  # MEB GEN2(2024+, ID.4 MK2 등): 일부 RX 메시지는 CRC를 "특정 길이까지만" + 새 매직으로 계산.
  # 수신값(d[0])과 일치하면 GEN2 방식 채택, 아니면 기존 방식 폴백 (infiniteCable2 동일 로직).
  # TX 메시지는 GEN2 테이블에 없어 항상 기존 방식으로 패킹 -> MK1 경로/패킹 불변.
  entry = VOLKSWAGEN_MQB_MEB_GEN2_CONSTANTS.get(address)
  if entry:
    trunc = d[:entry["length"]]
    crc = 0xFF
    for i in range(1, len(trunc)):
      crc ^= trunc[i]
      crc = CRC8H2F[crc]
    counter = d[1] & 0x0F
    crc ^= entry["magic"][counter]
    crc = CRC8H2F[crc]
    crc ^= 0xFF
    if crc == d[0]:
      return crc
...
VOLKSWAGEN_MQB_MEB_CONSTANTS: dict[int, list[int]] = {
    ...
    0x25D: [0xDA, 0x6B, 0x0E, 0xB2, 0x78, 0xBD, 0x5A, 0x81,
            0x7B, 0xD6, 0x41, 0x39, 0x76, 0xB6, 0xD7, 0x35],  # KLR_01 (capacitive wheel touch)
...
# MEB GEN2(2024+) 신형 CRC: 계산 길이 + 매직 (infiniteCable2에서 이식)
