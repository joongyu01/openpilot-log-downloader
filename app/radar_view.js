...
export function attachRadarReview(video) {
  ...
  plots.innerHTML = `<div class="radar-plot-legend"><span style="color:#f68e37">L1 거리 (레이더)</span><span style="color:#f5d348">L2 거리</span><span style="color:#4891ff">비전 거리 / 비전 L1</span><span style="color:#f75ea0">SCC 거리·가속도</span><span style="color:#3ecd82">L1 속도·Carrot 목표 가속도</span></div>
    ...
    <p class="muted tiny">기록된 Lead는 녹화 당시 차량의 판정입니다. 재계산은 이 컴퓨터의 검증 코드로 실행하며 민감도는 3으로 고정됩니다. 로그를 외부 서버에 업로드하지 않습니다. 레이더 소스·좌우 선택은 이 화면의 분석에만 적용되며 차량 설정은 바꾸지 않습니다. 좌우 반전은 전방 트랙의 위치·횡속도에 적용하고 SCC·코너·비전과 기록된 Lead는 유지합니다.</p>`;
  ...
  function drawGraph() {
    if(graph.clientWidth<=0||graph.clientHeight<=0)return;
  ...
  function draw() {
    if(!review.getClientRects().length||map.clientWidth<=0)return;
  ...
  async function load(selected) {
    ...
    const url=`/api/radar-review/${encodeURIComponent(selected.name)}?${query}`;
    try {
      const deadline=Date.now()+360000;
      let firstRequest=true;
      while(token===generation){
        const response=await fetch(url+(firstRequest?'&retry=1':''),{signal,cache:'no-store'});firstRequest=false;
        if(response.status===202||response.status===503){const progress=await response.json().catch(()=>({}));if(progress.message)setStatus(progress.message);if(Date.now()>deadline)throw new Error('분석 준비가 지연되고 있습니다. 잠시 후 다시 불러와 주세요.');await new Promise((resolve,reject)=>{const abort=()=>{clearTimeout(timer);reject(new DOMException('Aborted','AbortError'));};const timer=setTimeout(()=>{signal.removeEventListener('abort',abort);resolve();},2000);signal.addEventListener('abort',abort,{once:true});});continue;}
        if(!response.ok){const problem=await response.json().catch(()=>({}));throw new Error(problem.error||`레이더 데이터를 불러오지 못했습니다 (${response.status}).`);}
  ...
  const stop=()=>{generation++;controller?.abort();cancelAnimationFrame(frameHandle);running=false;video.pause();};
  draw();return {load, stop, cancel:stop};
}
